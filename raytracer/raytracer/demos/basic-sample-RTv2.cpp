...
namespace
{
    ...
    protected:
        ...
        std::vector<raytracer::LightSource> create_light_sources(TimeStamp now) override
        {
			...
			light_sources.push_back(raytracer::lights::omnidirectional(Point3D(0, 10, 10), colors::green()));
			light_sources.push_back(raytracer::lights::omnidirectional(Point3D(10, 0, 0), colors::cyan()));
...
void demos::basic_sample_RTv2(std::shared_ptr<pipeline::Consumer<std::shared_ptr<Bitmap>>> output)
{
